...
int main(int argc, char * argv[])
{
    ...
    if (use_ufs) {
        ...
        ret = toku_fs_mount(TOKUFS_MOUNT);
        assert(ret == 0);
        file = &tokufs_file;
        file->path = malloc(strlen(output_file) + 2);
        file->path[0] = '/';
        strcpy(file->path + 1, output_file);
    }
