...
static char * opt_string = "vhuc:f:s:n:d:m:b:x:";
...
static void usage(void)
{
    ...
    "    -u, --use-posix\n"
    "        perform benchmark on the underlying filesystem\n"
    "    -f, --dir\n"
...
static int posix_tokufs_open(const char * path, int flags, ...)
{
    return toku_fs_open(path, flags, 0644);
}

static struct benchmark_file_ops tokufs_file =
{
    .open = posix_tokufs_open,
    .close = toku_fs_close,
    .pwrite = toku_fs_pwrite,
    .pread = toku_fs_pread,
    .unlink = toku_fs_unlink,
    .mkdir = toku_fs_mkdir,
    .rmdir = toku_fs_rmdir
};
        ...
        const char * parent, int n, struct leaf_directories * leaves)
{
    ...
    for (int i = 0; i < directory_max_children && n > 0; i++) {
        ...
        int ret = file_ops->mkdir(subdir, 0755);
        assert(ret == 0);
...
static void benchmark_directory_scan_tokufs(char * parent, 
        struct benchmark_directory_scan_info * info)
{
    int ret;
    struct toku_dircursor cursor;

    // for simplicity, recursion is not done on non-directories.
    // so this call must succeed
    ret = toku_fs_opendir(parent, &cursor);
    assert(ret == 0);

    // four pages at each level of recursion
    int bufsize = 4 * getpagesize();
    ...
    do {
        int entries_read = 0;
        ret = toku_fs_readdir(&cursor, dirents, num_entries, &entries_read);
        if (ret == 0) {
        ...
        for (int i = 0; i < entries_read; i++) {
            ...
            // recur on this child if it is a directory, otherwise
            // count this child's into the total. tokufs does
            // not understand relative paths, so its correct to
            // just pass it the full path provided by the dirent
            if (S_ISDIR(d->st.st_mode)) {
                benchmark_directory_scan_tokufs(d->filename, info);
            } else {
                children_size += d->st.st_size;
            }
            free(d->filename);
        }

        info->num_files += entries_read;
        info->total_size += children_size;
    } while (keep_reading);


    free(dirents);
    ret = toku_fs_closedir(&cursor);
    assert(ret == 0);
}
...
static void benchmark_directory_scan_posix(char * parent,
        struct benchmark_directory_scan_info * info)
{
    int ret;
    DIR * dir;
    ...
    while ((entry = readdir(dir)) != NULL && !stop) {
        ...
        if (S_ISDIR(st.st_mode)) {
            benchmark_directory_scan_posix(entry->d_name, info);
        } else {
...
static void benchmark_directory_scan(struct benchmark_file_ops * file_ops,
        struct benchmark_directory_scan_info * info)
{
    ...
    if (file_ops == &tokufs_file) {
        benchmark_directory_scan_tokufs(benchmark_root_dir, info);
    } else {
        assert(file_ops == &posix_file);
        benchmark_directory_scan_posix(benchmark_root_dir, info);
    }
}
...
static void do_meta_scan(struct benchmark_file_ops * file_ops)
{
    long start, end;
    ...
    benchmark_directory_scan(file_ops, &scan_info);
...
static void run_benchmarks(struct benchmark_file_ops * file_ops)
{
    ...
    // underneath it
    ret = file_ops->mkdir(parent, 0755);
    if (ret != 0) {
        int e = errno;
...
int main(int argc, char * argv[])
{
    ...
    int pgsize = use_posix ? getpagesize() : (int) toku_fs_get_blocksize();
    ...
    printf(" * cachesize: %lu MB\n", use_posix ? 0 : cachesize / (1024*1024));
    printf(" * verbose? %s\n", verbose ? "yes" : "no");
    ...
    if (use_posix) {
        ...
        ret = toku_fs_mount("bstore-env.mount");
        assert(ret == 0);
        file_ops = &tokufs_file;
    }

    run_benchmarks(file_ops);

    if (!use_posix) {
        long start = toku_current_time_usec();
        ret = toku_fs_unmount();
        assert(ret == 0);
        printf("tokufs unmount time: %ld\n", toku_current_time_usec() - start);
    }
